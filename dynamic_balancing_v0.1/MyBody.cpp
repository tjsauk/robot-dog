...
void MyBody::update(unsigned long mills)
{
  ...
  static unsigned int wait_until = 0;
  //this function is to be called constantly in the main loop to update motor statemachines and changes in steering sPositions
...
void MyBody::moveCOMbeforeWalk(unsigned long mills)
{
...
coords MyBody::calculateMovementParameters(coords target, coords current) {
...
void MyBody::PositionUpdate(unsigned long mills)
{
  ...
  if (sDirection_x != 0 || sDirection_z != 0 || sDegree_y != 0)
  {
    ...
    //my_log(MYLOG_INFO, "Leg percentages FL FR RL RR: [%f, %f, %f, %f]", perc_FL,perc_FR,perc_RL,perc_RR);
    ...
    HandlePosition(total_step_points);//handle the possible stance changes before
   ...
   //nyt tässä käy siten että jokaisella dostepillä tulee vakio asento plus askelmuutos. ja myöhemmin lisätään koordinaatteja jotka nollaantuu seuraavalla dostepillä koska ne ei ole tallessa
   //pitää siis muuttaa siten että COM liike hommelit lisätään pysyvään asentoon
...
void MyBody::GoTestPosition(unsigned long mills, unsigned long duration, double ramp)
{//goes to the walk position set by configuration
double xo = -45;//Takajalat ilmassa -45 ja vastakkainen jalka y=-15 sama y mutta x = 5 etujaloille
double yo = 0;
double zo = WALK_POSITION_Z;
  mLeftFront.GoPosition(mills, xo, WALK_POSITION_FRONT_Y-15, zo, duration,ramp);
  mLeftRear.GoPosition(mills, xo, WALK_POSITION_REAR_Y, zo, duration,ramp);
  mRightFront.GoPosition(mills, xo, WALK_POSITION_FRONT_Y, -zo, duration,ramp);
  mRightRear.GoPosition(mills, xo, WALK_POSITION_REAR_Y-20, -zo, duration,ramp);
  //COMtobody();
