...
//define step size
  if (x+z == 0)
  {
    step_lenght = STEP_SIZE*y_rot/2;
  }else if (y_rot != 0)
  {
    //distance to turning radius origo from body origo
    double D0 = sqrt(pow(rotation[0], 2) + pow(rotation[1], 2));
    //distance from turning radius origo to leg position
    double D1 = sqrt(pow(rotation[0]-mPositionPaw.x, 2) + pow(rotation[1]-mPositionPaw.y, 2));
    step_lenght = STEP_SIZE*sqrt(pow(x,2)+pow(z,2))/2*(D1/D0);
  }else
  {
    step_lenght = STEP_SIZE*sqrt(pow(x,2)+pow(z,2))/2;
  }
  double viereinen = (rotation[0]-mPositionPaw.x+x_compensate);
  double vastainen = (rotation[1]-mPositionPaw.z+z_compensate);
  double hypotenuusa = sqrt(pow(viereinen, 2) + pow(vastainen, 2));

  if (y_rot == 0)//not turning just going straight
  {
    step_size_x = STEP_SIZE*x/2;
...
void MyLeg::addToWalk(double x, double y, double z)
{
  ...
  mPositionPaw.x = mPositionPaw.x+x ;
  mPositionPaw.y = mPositionPaw.y+y ;
  if (mLeft)
  {
...
{
  //this adds offset during temperary movement. For example when walking and robot needs to change its position when balancing
  mPositionPaw.x = mPositionPaw.x+delta.x ;
  mPositionPaw.y = mPositionPaw.y+delta.y ;
  if (mLeft)
  {
